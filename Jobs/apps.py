...
class JobsConfig(AppConfig):
    ...
    def ready(self):
        ...
        from .signals import run_job_on_queued, setup_python_script_on_create, \
            ...
            process_doc_on_create_atomic, update_digraph_on_edge_change, update_digraph_position_on_node_change, \
        ...
        # If a pipeline node is updated, check to see if its position changed and,
        # if so, update the digraph of its parent pipelne
        pre_save.connect(update_digraph_position_on_node_change, sender=PipelineNode, dispatch_uid=uuid.uuid4())
        ...
        # the digraph
        post_save.connect(update_digraph_on_node_create, sender=PipelineNode, dispatch_uid=uuid.uuid4())

        # After a pipeline step is deleted, regenerate the linked digraph (as with post_save, we don't care about node
        # meta data.
        post_delete.connect(update_digraph_on_node_delete, sender=PipelineNode, dispatch_uid=uuid.uuid4())
